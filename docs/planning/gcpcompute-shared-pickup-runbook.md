...
- `latest.md`에 triage, readiness, bridge, queue/backlog, pending approvals, repeated warnings, phrase-breath trend, harmonic-color trend, shadow reranker, orchestration trends, incident draft, escalation, comms draft section이 들어간다.
...
- incident lane이면 latest bundle에 incident draft metadata와 escalation/comms handoff가 같이 들어가고, incident draft가 phrase-breath, harmonic-color, shadow reranker, 또는 orchestration snapshot을 갖고 있으면 pickup JSON도 그 nested field를 그대로 보존한다.
