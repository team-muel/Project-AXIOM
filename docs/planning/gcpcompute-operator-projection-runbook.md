...
- `latest.json`의 `overseer.learnedBackboneBenchmark`는 active blind-review pack이 있으면 `reviewPackRecordActions[]`를 같이 싣는다. 이 field는 `recentActivePacks[].reviewSheetPath`에서 파생된 worksheet ingest command를 담으며, active pack이 있을 때는 duplicate pack generation action보다 먼저 읽어야 한다.
- 같은 object의 `reviewSampleStatus`는 이제 reviewed floor 자체뿐 아니라 `remainingReviewedRunCountForScreening`, `remainingReviewedRunCountForPromotion`, `remainingReviewedDisagreementCountForPromotion`도 같이 싣는다. projection consumer는 raw minimum과 현재 reviewed count를 다시 빼지 않고도 C-stage evidence gap을 바로 읽을 수 있다.
- 같은 object의 `pairedSelectionOutcomes`, `selectedWorkerOutcomes`, `coverageRows[]`는 reviewed promoted-vs-heuristic cohort outcome, selected worker별 reviewed cohort summary, benchmark별 coverage matrix를 compact하게 싣는다. projection consumer는 raw benchmark JSON을 다시 열지 않고도 reviewed cohort balance, worker별 outcome skew, benchmark별 pending review pressure, selected worker mix를 바로 읽을 수 있다.
