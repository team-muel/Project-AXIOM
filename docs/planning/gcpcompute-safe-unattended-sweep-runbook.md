...
- learned-backbone benchmark `reviewQueue`에 pending blind review가 있으면 `latest.json`, incident draft JSON, `latest.md`, incident markdown의 `## Learned Backbone Benchmark` section에 `reviewPack` action line이 추가된다. 이 line은 generic shortlist or pairwise queue용 `npm run ml:review-pack:learned-backbone -- --pendingOnly --reviewTarget=...` command뿐 아니라 custom budget cell용 `--searchBudget="custom(3)"` 또는 `--searchBudget="custom(3+1)"` command와 pending pair count도 함께 실어 budget-scoped review routing을 바로 넘긴다.
- learned-backbone blind review pack이 이미 생성되어 있으면 같은 section에 `reviewPacks` summary line도 같이 남는다. 여기에는 matched pack count, active pack count, pending worksheet decision count, latest generated or reviewed timestamp가 들어가므로 operator가 새 pack 생성 전에 기존 worksheet backlog가 남아 있는지 바로 읽을 수 있다.
- active learned-backbone review pack row는 `reviewSheet` path와 `npm run ml:review-pack:record:learned-backbone -- --resultsFile ...` ingest command를 같이 남긴다. unattended sweep은 기본적으로 duplicate blind-review pack 생성 action보다 이 worksheet ingest path를 먼저 보여 주지만, active pack들이 current pending blind-review queue를 전부 덮지 못하면 uncovered pending pair 수만큼 supplemental `reviewPack` action도 같이 남긴다.
...
- learned backbone benchmark aggregate에 retry drift나 directional-only sample이 남아 있으면 recommendation도 `learned backbone benchmark ...` advisory를 남긴다. directional-only 상태에서는 absolute minimum만 반복하지 않고 `reviewSampleStatus.remainingReviewedRunCountForPromotion`와 `remainingReviewedDisagreementCountForPromotion` 기반의 remaining gap을 같이 읽는다. 이 문구는 Stage A/C gate sample이 아직 얼마나 부족한지와 retry localization drift 여부를 함께 보고 backbone widening 판단을 미루라는 뜻이다.
...
- projection latest가 `overseer.learnedBackboneBenchmark`를 가지고 있으면 unattended sweep latest와 incident draft latest도 같은 compact snapshot과 `## Learned Backbone Benchmark` section을 포함한다. 여기서는 `reviewSampleStatus`, `blindPreference`, `reviewedTop1Accuracy`, `pairedSelectionOutcomes`, `selectedWorkerOutcomes`, `coverageRows`, `promotionGate`, `retryLocalizationStability`, `promotionAdvantage`, advisory line을 같이 읽어 benchmark sample sufficiency, reviewed promoted-vs-heuristic cohort balance, worker별 outcome skew, benchmark별 pending pressure, selected top-1 quality, retry drift, 그리고 guarded promotion hold 여부를 incident handoff까지 그대로 보존한다.
...
- incident draft learned backbone benchmark snapshot present: projection latest의 benchmark sample sufficiency와 retry drift snapshot이 incident handoff에도 보존됨
...
- learned backbone benchmark advisory present: benchmark sample floor 부족이나 retry drift가 있으면 recommendation이 widening hold guidance를 남긴다
